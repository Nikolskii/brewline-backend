...
| `CORS_ORIGINS`      | `http://localhost:5173,http://localhost:5174` | Origin'ы фронтов, которым браузер разрешит читать ответы      |
...
заголовками. Список задаётся явно и через запятую. Звёздочка здесь не работает принципиально:
